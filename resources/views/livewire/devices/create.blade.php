<section class="w-full">
    ...
    <x-devices.layout :heading="$isEditing ? __('Edycja urządzenia') : __('Dodawanie urządzenia')" :subheading="$isEditing ? __('Edytuj szczegóły urządzenia') : __('Dodaj nowe urządzenie do systemu')">
        <div class="my-6 w-full">
            ...
            <form wire:submit.prevent="saveDevice" class="space-y-6">
                <!-- Klient (dla admina/pracownika) -->
                @if(auth()->check() && auth()->user()->hasRole(['Admin', 'employee']))
                    <div>
                        <flux:select wire:model="user_id" :label="__('Klient')" :filter="false">
                ...
                <div class="flex items-center justify-end gap-4 pt-4">
                    ...
                    <flux:button type="submit">
                        {{ $isEditing ? __('Zapisz zmiany') : __('Dodaj urządzenie') }}
                    </flux:button>
                </div>
            </form>
        </div>
        ...
        <!-- Modal skanera QR -->
        @if($showQrScanner)
            <flux:modal name="confirm-user-account-deletion" :show="true" focusable class="max-w-lg">
